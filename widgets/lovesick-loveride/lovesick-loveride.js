{
  ...
  function handlePlayer() {
    ...
    for (let i = coins.length - 1; i >= 0; i--) {
      ...
      if (distanceFromPlayer <= coinSize / 2) {
        engine.addScore((1 + movement * 2) * 100);
        coins[i].remove();
      }
    }
  ...
  function mouseClicked() {
    ...
    if (scene == 0) {
      if (playerState == 0 && scene == 0) {
        ...
        // leaderboard
        if (collidePointRect(mouseX, mouseY, 216, 24, 72, 84))
          window.open('./leaderboard', '_blank').focus();
      }
      playerState = 0;
    }
    // game
  ...
  function draw() {
    ...
    if (scene == 0) {
      ...
      // leaderboard
      if (collidePointRect(mouseX, mouseY, 216, 24, 72, 84))
        image(menuAssets[7].img, 216, 24, 72, 84);
      else image(menuAssets[3].img, 216, 24, 72, 84);
    ...
    // game
      ...
      if (scene != -1) {
        ...
        textSize(40);
        fill(0);
        text(int(engine.getScore()), 950, 100);
        fill(217, 176, 28);
        text(int(engine.getScore()), 952, 102);
        fill(0);
        text('MULTI', 725, 100);
        fill(217, 176, 28);
        text('MULTI', 727, 102);
  ...
  function gameOver() {
    ...
    textAlign(CENTER, CENTER);
    textSize(min(18 * Math.log10(engine.getScore() + 10000), 110));
    fill(223, 191, 31);
    stroke(255);
    strokeWeight(2);
    ...
    textFont(font);
    text(engine.getScore(), width / 2, 600);
    engine.resetScore();
    noLoop();
    textFont(font2);
  }
